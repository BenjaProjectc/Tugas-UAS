...
st.title("📊 E-Commerce Comprehensive Dashboard")
...
folder_path = "C:/Users/benja/Downloads/Semester 3/PSD/UAS DASHBOARD/Notebooks/E-Commerce Public Dataset/Feature Engineered Dataset/"
dataset_path = os.path.join(folder_path, "feature_engineered_cleaned_sellers_dataset.csv")
...
if menu == "Exploratory Data Analysis (EDA)":
    ...
    **Penjelasan:** Bagian ini bertujuan untuk memahami karakteristik dasar dataset melalui statistik deskriptif, distribusi data, dan korelasi antar variabel.
    ...
    selected_file = st.sidebar.selectbox("Pilih file CSV:", list(dataframes.keys()))
    df_eda = dataframes[selected_file]

    # Tab untuk visualisasi
    tab1, tab2, tab3 = st.tabs(["Data Preview", "Statistik Deskriptif", "Missing Values"])
    with tab1:
    ...
    fig = px.histogram(df_eda, x=selected_num_col, nbins=30, title=f"Distribusi {selected_num_col}")
    st.plotly_chart(fig)
    ...
    sns.heatmap(df_eda[num_cols].corr(), annot=True, cmap="coolwarm", fmt=".2f", linewidths=0.5)
    st.pyplot(plt)
...
# ====================
    ...
    **Penjelasan:** Model klasifikasi XGBoost digunakan untuk memprediksi apakah seorang penjual aktif atau tidak berdasarkan fitur-fitur seperti total pesanan dan transaksi bulanan.
    ...
    total_orders = st.sidebar.number_input("Total Orders", min_value=0, value=10)
    average_price = st.sidebar.number_input("Average Price", min_value=0.0, value=50.0)
    average_monthly_transactions = st.sidebar.number_input("Average Monthly Transactions", min_value=0.0, value=5.0)
    average_distance_to_customers = st.sidebar.number_input("Average Distance to Customers", min_value=0.0, value=100.0)
    seller_count_by_city = st.sidebar.number_input("Seller Count by City", min_value=0, value=50)
    seller_count_by_state = st.sidebar.number_input("Seller Count by State", min_value=0, value=200)
    ...
    st.sidebar.write(f"### Prediksi: {predicted_label}")
